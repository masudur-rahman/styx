...
func (m *MockDB) FindOne(document interface{}, filter ...interface{}) (bool, error) {
	if err := dberr.CheckEntityNameNonEmpty(m.entity); err != nil {
		return false, err
	}
	if err := dberr.CheckIdOrFilterNonEmpty(m.id, filter); err != nil {
		return false, err
	}
	if filter == nil {
...
func (m *MockDB) UpdateOne(document interface{}) error {
	...
	if err := dberr.CheckIDNonEmpty(m.id); err != nil {
		return err
	}
